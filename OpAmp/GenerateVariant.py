...
# Import the sizing functions from sizing_lib.py
import sys
from pathlib import Path

# Add current directory to path to import sizing_lib
sys.path.insert(0, str(Path(__file__).parent))

try:
    import sizing_lib
    Specifications = sizing_lib.Specifications
    SquareLawParams = sizing_lib.SquareLawParams
    DesignChoices = sizing_lib.DesignChoices
    design_square_law = sizing_lib.design_square_law
    design_gmid = sizing_lib.design_gmid
    SKY130_MIN_L = sizing_lib.SKY130_MIN_L
    LOOKUP_DIR = sizing_lib.LOOKUP_DIR
except ImportError as e:
    print(f"Error: Could not import sizing_lib module: {e}")
    print("Make sure sizing_lib.py exists in the same directory")
    sys.exit(1)
